...
class GameViewModel(
    ...
    private fun initData() {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            awaitAll(
                gameDataCollected,
                playerDataCollected,
                if (state.value.isRunner) activeRunnerZonesCollected else runnerDataCollected
            )
    ...
    private fun updateActiveRunnerZones(zoneIds: List<Int>, nextUpdate: Long) {
        ...
        _state.update {
            it.copy(
                activeRunnerZones = activeZones,
                zoneUpdateTime = delay,
            )
        }
    ...
    private fun updateRunner(runner: Runner) {
        ...
        _state.update {
            it.copy(
                runner = runner,
                runnerLocationUpdateTime = delay,
            )
        }
